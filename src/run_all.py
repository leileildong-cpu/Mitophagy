...
class UltraFastVirtualScreening:
    ...
    def _compute_fingerprint_similarity_ultra_fast(self, ref_fps, lib_fps) -> np.ndarray:
        """超高速指纹相似性计算"""
        # PyTorch GPU计算（最优先）
        if TORCH_AVAILABLE and torch.is_tensor(ref_fps) and torch.is_tensor(lib_fps):
            try:
                with torch.no_grad():
                    # Tanimoto相似性的向量化计算
                    intersection = torch.mm(lib_fps, ref_fps.T)
                    lib_sum = torch.sum(lib_fps, dim=1, keepdim=True)
                    ref_sum = torch.sum(ref_fps, dim=1, keepdim=True)
                    union = lib_sum + ref_sum.T - intersection

                    tanimoto = intersection / (union + 1e-8)
                    return tanimoto.cpu().numpy()

            except Exception as e:
                print(f"[WARNING] PyTorch指纹超高速计算失败: {e}")

        # CuPy GPU计算（备选）
        if GPU_AVAILABLE:
            try:
                if not (hasattr(ref_fps, '__array__') and hasattr(lib_fps, '__array__')):
                    ref_array = np.array([fp for fp in ref_fps if fp is not None], dtype=np.float32)
                    lib_array = np.array([fp for fp in lib_fps if fp is not None], dtype=np.float32)
                else:
                    ref_array = np.asarray(ref_fps)
                    lib_array = np.asarray(lib_fps)

                ref_gpu = cp.asarray(ref_array, dtype=cp.float32)
                lib_gpu = cp.asarray(lib_array, dtype=cp.float32)

                # Tanimoto相似性
                intersection = cp.dot(lib_gpu, ref_gpu.T)
                lib_sum = cp.sum(lib_gpu, axis=1, keepdims=True)
                ref_sum = cp.sum(ref_gpu, axis=1, keepdims=True)
                union = lib_sum + ref_sum.T - intersection

                tanimoto = intersection / (union + 1e-8)
                return cp.asnumpy(tanimoto)

            except Exception as e:
                print(f"[WARNING] CuPy指纹超高速计算失败: {e}")

        # CPU向量化回退
        return self._compute_fingerprint_similarity_cpu_vectorized(ref_fps, lib_fps)
    ...
    def aggregate_similarities_ultra_fast(self, similarities: Dict) -> Dict:
        """
        改进的相似性聚合：使用最佳匹配策略（取最大值而非平均值）
        """
        print("[INFO] 聚合多维相似性（最佳匹配策略）...")
        ...
        print(f"  - 1D: {sim_1d.shape if hasattr(sim_1d, 'shape') else type(sim_1d)}")
        print(f"  - 2D: {sim_2d.shape if hasattr(sim_2d, 'shape') else type(sim_2d)}")
        print(f"  - 3D: {sim_3d.shape if hasattr(sim_3d, 'shape') else type(sim_3d)}")

        # 检查是否是矩阵（库分子数 x 参考分子数）
        if hasattr(sim_1d, 'shape'):
            print(f"[DEBUG] 期望形状: (11301, 14) - 实际: {sim_1d.shape}")
        # ===================================

        # 处理不同类型的数据（numpy, torch tensor, cupy）
        if TORCH_AVAILABLE and torch.is_tensor(sim_1d):
            print("[DEBUG] 使用PyTorch路径")
            with torch.no_grad():
                # ========== 添加调试 ==========
                print(f"[DEBUG] 原始1D相似性范围: {sim_1d.min():.4f} - {sim_1d.max():.4f}")
                # =============================
                ...
                # ========== 添加调试 ==========
                print(f"[DEBUG] 取最大值后1D范围: {max_sim_1d.min():.4f} - {max_sim_1d.max():.4f}")
                print(f"[DEBUG] 取最大值前5个: {max_sim_1d[:5]}")
                # =============================
                ...
                # ========== 添加调试 ==========
                print(f"[DEBUG] 归一化后1D范围: {max_sim_1d.min():.4f} - {max_sim_1d.max():.4f}")
                # =============================
                ...
                max_sim_1d_np = max_sim_1d.cpu().numpy()
                max_sim_2d_np = max_sim_2d.cpu().numpy()
                max_sim_3d_np = max_sim_3d.cpu().numpy()

        elif GPU_AVAILABLE and hasattr(sim_1d, '__array__'):
            # CuPy GPU路径
            try:
                import cupy as cp
                if isinstance(sim_1d, cp.ndarray):
                    max_sim_1d = cp.max(sim_1d, axis=1)
                    max_sim_2d = cp.max(sim_2d, axis=1)
                    max_sim_3d = cp.max(sim_3d, axis=1)
                    ...
                    # 转换回numpy
                    combined_scores_np = cp.asnumpy(combined_scores)
                    max_sim_1d_np = cp.asnumpy(max_sim_1d)
                    max_sim_2d_np = cp.asnumpy(max_sim_2d)
                    max_sim_3d_np = cp.asnumpy(max_sim_3d)
                else:
                    # 回退到numpy
                    raise ValueError("Not a CuPy array")
            except Exception as e:
                print(f"[WARNING] CuPy处理失败，使用CPU: {e}")
                # 回退到numpy处理
                ...
                max_sim_1d_np = self._minmax_normalize(max_sim_1d)
                max_sim_2d_np = self._minmax_normalize(max_sim_2d)
                max_sim_3d_np = self._minmax_normalize(max_sim_3d)

                w1 = self.config["similarity"]["w_1d"]
                w2 = self.config["similarity"]["w_2d"]
                w3 = self.config["similarity"]["w_3d"]

                combined_scores_np = w1 * max_sim_1d_np + w2 * max_sim_2d_np + w3 * max_sim_3d_np
        else:
            # CPU numpy路径
            if not isinstance(sim_1d, np.ndarray):
                sim_1d = np.asarray(sim_1d)
                sim_2d = np.asarray(sim_2d)
                sim_3d = np.asarray(sim_3d)

            # 取最大值（每行的最大值 = 每个库分子与所有参考分子的最大相似性）
            max_sim_1d = np.max(sim_1d, axis=1) if len(sim_1d.shape) > 1 else sim_1d
            max_sim_2d = np.max(sim_2d, axis=1) if len(sim_2d.shape) > 1 else sim_2d
            max_sim_3d = np.max(sim_3d, axis=1) if len(sim_3d.shape) > 1 else sim_3d
        ...
        # 打印统计信息
        print(f"[INFO] 得分统计 (最佳匹配策略):")
        print(f"  - 最高得分: {combined_scores_np.max():.4f}")
        print(f"  - 平均得分: {combined_scores_np.mean():.4f}")
        print(f"  - 中位数得分: {np.median(combined_scores_np):.4f}")
        print(f"  - 75分位数: {np.percentile(combined_scores_np, 75):.4f}")
        print(f"  - 90分位数: {np.percentile(combined_scores_np, 90):.4f}")

        return {
            'combined_scores': combined_scores_np,
            'individual_scores': {
                '1d_max': max_sim_1d_np if 'max_sim_1d_np' in locals() else max_sim_1d.cpu().numpy() if TORCH_AVAILABLE and torch.is_tensor(
                    max_sim_1d) else max_sim_1d,
                '2d_max': max_sim_2d_np if 'max_sim_2d_np' in locals() else max_sim_2d.cpu().numpy() if TORCH_AVAILABLE and torch.is_tensor(
                    max_sim_2d) else max_sim_2d,
                '3d_max': max_sim_3d_np if 'max_sim_3d_np' in locals() else max_sim_3d.cpu().numpy() if TORCH_AVAILABLE and torch.is_tensor(
                    max_sim_3d) else max_sim_3d,
                '1d_norm': max_sim_1d_np,
                '2d_norm': max_sim_2d_np,
                '3d_norm': max_sim_3d_np
            }
        }
