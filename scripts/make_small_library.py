...
def load_csv_like(path: str, smiles_col: Optional[str]=None, id_col: Optional[str]=None, topn: int=-1) -> List[Tuple[str, Optional[str]]]:
    """读取 .csv 或 .csv.gz"""
    if pd is None:
        raise RuntimeError("需要 pandas 才能读取 CSV")

    # 如果指定了topn，读取时就限制行数（多读一些以防有无效数据）
    nrows = None if topn <= 0 else topn *2
    ...
    for kwargs in [
        dict(nrows=nrows),
        dict(sep=';', nrows=nrows),
        dict(encoding='utf-8-sig', nrows=nrows),
        dict(sep=';', encoding='utf-8-sig', nrows=nrows),
        dict(sep='\t', nrows=nrows),
    ]:
        try:
...
def main():
    ...
    # 加载原始条目（现在会根据topn限制读取行数）
    try:
        raw_items = load_csv_like(in_path, smiles_col=args.smiles_col, id_col=args.id_col, topn=topn)
    except Exception as e:
    ...
    with open(out_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for rid, smi, source in records:
            writer.writerow([rid, source, smi])
