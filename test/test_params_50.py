...
def test_different_weights():
    ...
    for w1, w2, w3, name in weight_tests:
        ...
        config['output']['dir'] = f'outputs/param_50_{name}'
        
        config_file = f'config_50_{name}.yaml'
        ...
        try:
            ...
            if result.returncode == 0:
                # 分析结果
                output_dirs = [d for d in os.listdir('outputs') if d.startswith(f'param_50_{name}_')]
                if output_dirs:
    ...
    results_df.to_csv('outputs/weight_test_50_results.csv', index=False)
